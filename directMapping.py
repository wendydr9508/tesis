...
#SQLAlchemy is a library that facilitates the communication between Python programs and databases. Most of the times,
#this library is used as an ORM tool that translates Python classes to tables on relational databases and automatically
#converts function calls to SQL statements.
# search google sqlalchemy-orm-tutorial-for-python-developers/
...
def orm_automap_base(name='Base', base_iri=None, mapper=None,
    ...
    def prepare(cls, engine=None, reflect=False, **kwargs):
        ...
        prepare_base(engine=engine, reflect=True, **kwargs)
        for class_ in AutomapBase.classes:
...
class OrmRdfMapper(object):
    ...
    def node(self):
        try:
            ...
            if self.has_pseudo_primary_key:
                ...
                pkey_rdf_items = \
                    [(tiposComunes.iri_safe(col.name),
                      tiposComunes.iri_safe
                       (tiposComunes.rdf_literal_from_sql(col.name,
                                                     sql_type=col.type)))
                     for col in self.class_.primary_key]
                self._node = \
    ...
    def has_pseudo_primary_key(self):
        return self.class_.has_pseudo_primary_key
        ...
        try:
            ...
            for col in self.__table__.columns:
                ...
                if value is not None:
                    value_rdf = tiposComunes.rdf_literal_from_sql(value,
                                                             sql_type=col.type)
    ...
    def table_iri(cls):
        try:
            ...
            cls._table_iri = \
                cls._prefixed_iri\
                 (_rdf.URIRef(tiposComunes.iri_safe(cls.name)))
            return cls._table_iri
